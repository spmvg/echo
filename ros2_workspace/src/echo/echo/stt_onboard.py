...
# Audio capture is moved to a separate node that publishes base64 audio chunks on /audio/raw
...
class STTOnboard(Node):
    ...
    def __init__(self):
        ...
        # Queue shared between ROS subscriber callback and the listen loop
        self.audio_q = LifoQueue()

        # Subscribe to raw audio published by the separate audio capture node
        self.create_subscription(String, '/audio/raw', self._audio_sub_cb, 100)
    ...
    def _audio_sub_cb(self, msg: String):
        """Decode base64 audio chunk and put raw bytes into the queue used by the recognizer.

        The audio_capture node publishes frames as base64-encoded raw int16 PCM bytes.
        """
        raw = b64decode(msg.data)
    ...
    def _listen_loop(
        ...
        self.get_logger().info(f"Listening for wake word '{wake_word}' (audio from /audio/raw)...")
